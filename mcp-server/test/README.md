...
| File | Assertion |
|------|-----------|
| `guard.test.js` | The **live** registered tool surface (obtained via a real `tools/list` request) exposes **only** `browser_`/`devtools_`/`keyboard_`/`dev_` tools; **zero** sensitive-family tools register; `browser_fill_password` (the historical escaped-guard leak) is locked out **by name**; no tool name embeds a sensitive token (e.g. `password`, `oauth`, `captcha`) even behind a safe prefix; the `BLOCKED_TOOL` guard wrapper is still present in `lib/mcp.js` (the registration seam every tool module flows through) and still rejects every blocked family. Tool count is pinned **exactly** to the locked clean-core number (`EXPECTED_TOOL_COUNT`, currently 138) **and** held under a one-way ceiling — an accidental add fails loudly. |
...
- It reads the **live registration** (a genuine MCP `tools/list` response), not a
  static scrape and not hard-coded line numbers — so it tracks the server source
  (`server.js` + `lib/` + `tools/` modules) as the backend trims it, and a
  re-added blocked tool fails the suite immediately.
- The expected safe prefixes, blocked-family probes, and sensitive name tokens
  live in `fixtures.mjs` as an **independent source of truth** (the clean-core
  contract, not imported from the server), so weakening `lib/mcp.js`'s own guard
  regex still fails the tests.
